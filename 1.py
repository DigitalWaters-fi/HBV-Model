...
def plot_and_select_catchment(shapefile_path):
    ...
    def onclick(event):
        # Ignore non-left clicks or double clicks
        ...
        # Close the figure automatically after a valid selection
        plt.close(fig)
...
def main():
    ...
    print(f"\nSelected catchment ID: {selected_catchment_id}")
    dinfo_path = input("Enter path to 'predata.csv' file: ").strip()

    # Run HBV preparation with user-selected shapefile and catchment
    prepare_meteorological_and_landuse_data(
