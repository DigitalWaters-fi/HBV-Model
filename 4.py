...
def choose_output_file(output_files):
    """Allow the user to choose which HBV output file to open."""
    print("\nHBV model outputs:")
...
def preview_and_select_column(csv_path):
    """Preview CSV contents and let user choose a column."""
    ...
    col_choice = int(input("\nEnter the number of the column you want to plot: "))
    selected_col = df.columns[col_choice - 1]
    print(f"\nYou selected column: '{selected_col}'")

    if not pd.api.types.is_numeric_dtype(df[selected_col]):
        raise ValueError(f"The selected column '{selected_col}' is not numeric. Please choose a numeric column.")

    return df, selected_col


def aggregate_and_plot_results(df, selected_col, shapefile_path, id_col, selected_catchment_id):
    """Aggregate user-selected column and plot it on the map."""
    agg_choice = input("\nWould you like to calculate 'sum' or 'mean' of this column? [sum/mean]: ").strip().lower()
    if agg_choice == "mean":
        total_value = df[selected_col].mean()
    else:
        total_value = df[selected_col].sum()

    print(f"\n{agg_choice.title()} of '{selected_col}' for selected catchment: {total_value}")

    gdf = gpd.read_file(shapefile_path)
    gdf['selected_value'] = 0.0
    gdf.loc[gdf[id_col] == selected_catchment_id, 'selected_value'] = total_value
    ...
    plt.title(f"{agg_choice.title()} of {selected_col} for Catchment ID {selected_catchment_id}")
    plt.show()
...
def main():
    ...
    print(output_files)
    selected_key, selected_path = choose_output_file(output_files)
    df, selected_col = preview_and_select_column(selected_path)
    aggregate_and_plot_results(df, selected_col, shapefile_path, id_col, selected_catchment_id)
